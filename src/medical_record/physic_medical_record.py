...
from src.utils.s3_utils import remove_files_from_bucket, upload_file_to_bucket, download_file_from_bucket


class PhysicMedicalRecord(MedicalRecord):
    ...
    def save(self, medical_record_local_path: str) -> None:
        ...
        values = (
                f"({self.patient_id}, {self.speciality_id}, '{s3_file_name}', false, " +
    ...
    def remove_expired_medical_records_data():
        try:
            ...
            medical_records_to_remove = PhysicMedicalRecord.retrieve_expired_medical_records()
    ...
    def retrieve_medical_record(self, medical_record_id: int) -> bool:
        try:
            mysql_connection = RDSService().mysql_connection
            sql_query = (
                f'select s3_file_name from stage.medical_record where ' +
                f'medical_record_id = {medical_record_id} and is_electronic = false'
            )
            with mysql_connection.cursor() as cursor:
                cursor.execute(query=sql_query)
                result = cursor.fetchone()
            if result is not None:
                download_file_from_bucket(s3_file_name=result.get('s3_file_name'))
                return True
            else:
